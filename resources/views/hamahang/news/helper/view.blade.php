            ...
            <table style="width: 100%;">
                <tr>
                    <td style="width: 15%; text-align: center; padding: 10px 10px 10px 25px; ">
                        ...
                        @if (!$post->isOwner)
                            <div style="text-align: center; vertical-align: top; padding-top: 50px;">
                                <i class="fa fa-sort-asc fa-4x vote-up{!! $post->id !!}" aria-hidden="true" data-target_id="{!! $post->id !!}" data-type="+1"
                                   style="cursor: pointer; color: {!! +1 == $post->hasVoted ? 'orange' : 'lightgrey' !!};"></i>
                                <span style="font-size: large;"><div class="vote-count{!! $post->id !!}" style="direction: ltr;">{!! strip_tags($post->voteSum) !!}</div></span>
                                <i class="fa fa-sort-desc fa-4x vote-down{!! $post->id !!}" aria-hidden="true" data-target_id="{!! $post->id !!}" data-type="-1"
                                   style="cursor: pointer; color: {!! -1 == $post->hasVoted ? 'orange' : 'lightgrey' !!};"></i>
                            </div>
                        @endif
                    </td>
        ...
        @foreach ($post->answers as $answer)
            ...
            <div class="data-delete{!! $answer->id !!}">
                <table style="width: 100%;">
                    <tr>
                        <td style="width: 15%; text-align: center; padding: 10px 10px 10px 25px; ">
                            {!! $answer->user->LargAvatar !!}<br/>
                            <div style="margin-top: 5px;"><a href="{{ url($answer->user->Uname) }}">{!! $answer->user->Name !!} {!! $answer->user->Family !!}</a></div>
                            @if ($answer->accepted)
                                <br/>
                                <i class="fa fa-check-circle fa-4x" aria-hidden="true" style="color: #3eb332;"></i>
                            @elseif (!$post->accepted && $post->isOwner)
                                <br/>
                                <i class="fa fa-check-circle fa-4x clickable-accept" aria-hidden="true" style="cursor: pointer; color: lightgray;" postid="{!! $answer->id !!}"></i>
                            @endif
                            @if (!$answer->isOwner)
                                <div style="text-align: center; vertical-align: top; padding-top: 20px;">
                                    <i class="fa fa-sort-asc fa-4x vote-up{!! $answer->id !!}" aria-hidden="true" data-target_id="{!! $answer->id !!}" data-type="+1"
                                       style="cursor: pointer; color: {!! +1 == $answer->hasVoted ? 'orange' : 'lightgrey' !!};"></i>
                                    <span style="font-size: large;"><div class="vote-count{!! $answer->id !!}" style="direction: ltr;">{!! strip_tags($answer->voteSum) !!}</div></span>
                                    <i class="fa fa-sort-desc fa-4x vote-down{!! $answer->id !!}" aria-hidden="true" data-target_id="{!! $answer->id !!}" data-type="-1"
                                       style="cursor: pointer; color: {!! -1 == $answer->hasVoted ? 'orange' : 'lightgrey' !!};"></i>
                                </div>
                            @endif
                        </td>
