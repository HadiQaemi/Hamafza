<script>
    ...
    function RemoveFromPackageStep2(id) {
        if(id == 'no')
        {
            ...
            $.ajax({
                type: "POST",
                url: '{{ URL::route('hamahang.tasks.my_tasks.remove_from_package') }}',
                dataType: "json",
                data: sendInfo,
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        package_tasks += '<li>' + data[i]['title'] + '<span class="pull-left cursor-pointer"><i class="fa fa-remove" onclick="RemoveFromPackage(' + data[i]['package_id'] + ',' + data[i]['utpid'] + ')' +
                            '"></i></span></li>';
                    }
                    package_tasks += '</ul>';
    ...
    function add_task_to_package() {
        if (selectedrow.length > 0) {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('hamahang.tasks.my_tasks.add_to_package') }}',
                dataType: "json",
                data: sendInfo,
                success: function (data) {
                    var package_tasks = '<ul>';
                    for (var i = 0; i < data.length; i++) {
                        package_tasks += '<li>' + data[i]['title'] + '<span class="pull-left"><i class="fa fa-remove cursor-pointer" onclick="RemoveFromPackage(' + data[i]['package_id'] + ',' + data[i]['utpid'] + ')' +
                            '"></i></span></li>';
                    }
                    package_tasks += '</ul>';
                    $('#package' + current_package).html(package_tasks);
