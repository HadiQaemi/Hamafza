...
<script>
    function search_ajax() {
        var respite_day_no = $('#respite').val();
        var started_tasks = '';
        ...
        $.ajax({
            type: "POST",
            url: '{{ URL::route('hamahang.tasks.custom_tasks_priority') }}',
            dataType: "json",
            data: sendInfo,
            success: function (data) {
                put_data(data['data']);
                draganddrop();
                $(".task_info2").click(function () {
                    show_task_info($(this).data("t_id"));
                });
            },
            err: function (data) {
                document.getElementById('type1_tasks').empty();
                document.getElementById('type2_tasks').empty();
                document.getElementById('type3_tasks').empty();
                document.getElementById('type4_tasks').empty();
            }
        });
        function put_data(tasks) {
            ...
            $('#type4_tasks').empty();
            for (task in tasks) {
                var responser = '<sapn style="color:lightpink;font-size: 11px;margin-right: 8px"><span>ارجاع دهنده : </span>'+ tasks[task]['full_name'] +'</span>';
                if (tasks[task]['immediate'] == 1 && tasks[task]['importance'] == 1) {
                    ...
                    txt += '</div><div class="task_title"><a class="task_info2 cursor-pointer" data-t_id = "'+tasks[task]['id']+'" >'+ tasks[task]['title'] + '</a>' + responser +'</div><div style="position: absolute; left: 7px;" class="state_icon">';
