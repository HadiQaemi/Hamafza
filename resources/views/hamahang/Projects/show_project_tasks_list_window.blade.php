...
function show_project($parents,$ordered_project_tasks,$id,$pid,$cnt=1)
{
    foreach($parents[$id] as $sub_task)
    {
        ...
        echo '<div class="col-xs-2"><i class="fa fa-floppy-o task_project_save_status pointer margin-left-10" tp="child" rel="'.$sub_task['rel'].'" t="'.$ordered_project_tasks[$sub_task['id']]->id.'" pid="'.$pid.'" parent="'.$id.'"></i>
<i class="fa fa-remove task_project_remove pointer margin-left-10" hp_task="'.enCode($ordered_project_tasks['hp_task']).'" rel="'.enCode($sub_task['rel']).'" t="'.$ordered_project_tasks[$sub_task['id']]->id.'"></i>
