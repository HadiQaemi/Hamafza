    ...
    function show_project($parents,$ordered_project_tasks,$id,$pid,$cnt=1)
    {
        foreach($parents[$id] as $sub_task)
        {
            ...
            echo '<div class="col-xs-2">
                ...
                <i class="fa fa-check task_project_save_status pointer margin-left-10" hp_task="'.(isset($ordered_project_tasks['hp_task']) ? enCode($ordered_project_tasks['hp_task']) : '').'" tp="child" rel="'.$sub_task['rel'].'" t="'.enCode($ordered_project_tasks[$sub_task['id']]->id).'" pid="'.enCode($pid).'" parent="'.$id.'"></i>';
