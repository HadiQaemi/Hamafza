<script>
    ...
    function send_user(e, event) {
        ...
        if (n > 2) {
            ...
            mytimeout=setTimeout($.post('{!! route('search_list_user') !!}', {'term': strs,selected_array:f}, function (data) {
    ...
    $(document).ready(function(){
        ...
        $('#html1').jstree({
            'core' : {
                'data' : [
                    {"id" : 1, "text" : "همه کاربران" ,"state" : { "selected" : true }},
...
$('#span_selected_select').click(function(){
    $("#SelectedUsers_div").hide();
    $("#div_loader").show();
    $("#div_count_selected").addClass('selected_element');
    $.post('{!! route('selected_list_user') !!}', {selected_array:f}, function (info) {

        $("#SelectedUsers_div").html(info);
        $("#SelectedUsers_div").show();
        $("#div_loader").hide();
    });
    instance.deselect_all()
    $("#SearchDiv").hide();
