...
class PostsClass
{
    ...
    public function UserWall($uid, $num)
    {
        ...
        $table = DB::table('posts as p')
            ...
            ->whereRaw($sr)
            ->orderBy('p.id', 'desc')
            ->select('p.title', 'v.gid', 'p.id', 'p.shid', 'p.uid', 'p.sid', 'p.type', 'p.likes', 'p.coms', 'p.shares', 'p.desc', 'p.pic', 'p.video', 'p.reg_date', 'u.id as uid', 'u.Uname', 'u.Name', 'u.Family', 'u.Pic AS Pic','u.avatar')
            ->take($num)
            ->get();
        $nums = 0;
        foreach ($table as $row)
